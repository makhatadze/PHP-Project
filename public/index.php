...
$router->post('/submit-signup', function (IRequest $request) use ($db) {
    ...
    if(
        ...
        redirect('/signup');
        echo "User Create Succesfull";
    }
});

$router->post('/submit-newjob', function (IRequest $request) use ($db,$router) {
    $body = $request->getBody();
    $db->setPost(currentUser()['id'],$body['cname'],$body['company'],$body['category'],$body['city'],$body['ganakveti'],$body['pirobebi']);
    redirect('/');
});
